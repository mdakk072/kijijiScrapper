...
from scraper.DataFormatter import DataFormatter


class ActionScraper(BaseRequest ):
    
    def __init__(self,**kwargs ):
        self.logger = Utils.get_logger()
        self.logger.debug("Initializing ActionScraper...")
        super().__init__()
        self.headers = kwargs.get('headers') or self.headers
        self.logger.debug("ActionScraper initialized.")
    ...
    def extract_ad_JSON(self, response):
        ...
        script_tag = soup.find('script', {'id': script_id})
        if not script_tag:
            self.logger.error("Script tag containing JSON data not found in the response.")
            raise RuntimeError("Script tag containing JSON data not found in the response.")

        data_text = script_tag.string
        try:
            data = json.loads(script_tag.string)
            ad_info = data.get("props", {}).get("pageProps", {}).get("componentProps", {}).get("adInfo", {}).get("ad", {})
    ...
    def get_ads_listings(self, json_data):
        ...
        try:
            ...
            listings = json_data.get('props', {}).get('pageProps', {}).get('initialReduxState', {}).get('ad', {}).get('search', {}).get('ads', [] )
